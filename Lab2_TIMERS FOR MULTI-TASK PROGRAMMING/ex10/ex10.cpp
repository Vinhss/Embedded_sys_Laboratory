...
extern "C" void TIMER0_IRQHandler(void) //Use extern "C" so C++ can link it properly, for C it is not required
{
	...
	if(mode==2)
	{
		switch(led_index)
		{  
			case 0: LPC_GPIO2->FIOPIN &=~ (1<<6); //TURN OFF LED7
							LPC_GPIO1->FIOPIN |= (1<<28); //TURN ON LED0
				   break;
			case 1: LPC_GPIO1->FIOPIN &=~ (1<<28); //TURN OFF LED
							LPC_GPIO1->FIOPIN |= (1<<29); //TURN ON LED1
				   break;			   
			case 2: LPC_GPIO1->FIOPIN &=~ (1<<29); //TURN OFF LED1
							LPC_GPIO1->FIOPIN |= (1<<31); //TURN ON LED2
				   break;
			case 3: LPC_GPIO1->FIOPIN &=~ (1<31); //TURN OFF LED2
							LPC_GPIO2->FIOPIN |= (1<<2); //TURN ON LED3
				   break;
			case 4: LPC_GPIO2->FIOPIN &=~ (1<<2); //TURN OFF LED3
							LPC_GPIO2->FIOPIN |= (1<<3); //TURN ON LED4
				   break;
			case 5: LPC_GPIO2->FIOPIN &=~ (1<<3); //TURN OFF LED4
							LPC_GPIO2->FIOPIN |= (1<<4); //TURN ON LED5
				   break;
			case 6: LPC_GPIO2->FIOPIN &=~ (1<<4); //TURN OFF LED5
							LPC_GPIO2->FIOPIN |= (1<<5); //TURN ON LED6
				   break;
			case 7: LPC_GPIO2->FIOPIN &=~ (1<<5); //TURN OFF LED6
							LPC_GPIO2->FIOPIN |= (1<<6); //TURN ON LED7
				   break;
			case 8: LPC_GPIO2->FIOPIN &=~ (1<<6); //TURN OFF LED7
					 break;
		}
	}
}
