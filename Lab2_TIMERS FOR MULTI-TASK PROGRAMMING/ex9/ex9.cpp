...
{
	/*Assuming that PLL0 has been setup with CCLK = 100Mhz and PCLK = 25Mhz.*/
	LPC_SC->PCONP |= (1<<2); //Power up TIM1. By default TIM0 and TIM1 are enabled.
	LPC_SC->PCLKSEL0 &= ~(0x3<<5); //Set PCLK for timer = CCLK/4 = 100/4 (default)
	LPC_TIM1->CTCR = 0x0;
...
{
	/*Assuming that PLL0 has been setup with CCLK = 100Mhz and PCLK = 25Mhz.*/
	LPC_SC->PCONP |= (1<<22); //Power up TIM2. By default TIM0 and TIM1 are enabled.
...
{
	/*Assuming that PLL0 has been setup with CCLK = 100Mhz and PCLK = 25Mhz.*/
	LPC_SC->PCONP |= (1<<23); //Power up TIM3. By default TIM0 and TIM1 are enabled.
	LPC_SC->PCLKSEL1 &= ~(0x3<<15); //Set PCLK for timer = CCLK/4 = 100/4 (default)
	LPC_TIM3->CTCR = 0x0;
	LPC_TIM3->PR = PRESCALE; //Increment LPC_TIM0->TC at every 24999+1 clock cycles
