...
void Joy_stick_scan(void)
{
	...
	if (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_UP))
	{
		...
		while (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_UP))
	...
	if (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_DOWN))
	{
		...
		while (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_DOWN))
			; // Wait until the button is released
	}
	...
	if (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_CENTER))
	{
		...
		while (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_CENTER))
			; // Wait until the button is released
	}
	...
	if (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_LEFT))
	{
		...
		while (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_LEFT))
			; // Wait until the button is released
	}
	...
	if (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_RIGHT))
	{
		...
		while (!(LPC_GPIO1->FIOPIN & MASK_JOYSTICK_RIGHT))
			; // Wait until the button is released
	}
}
...
void TimeSettingScreenJoystickControl(int joystickStatus)
{
	...
	case JOYSTICK_UP:
		...
		{
		case 0:
			Increase(&Now.Hour, 23);
			break;
		case 1:
			Increase(&Now.Minute, 59);
			break;
		case 2:
			Increase(&Now.Second, 59);
			break;
		default:
			break;
		}
	...
	case JOYSTICK_DOWN:
		...
		{
		case 0:
			Decrease(&Now.Hour, 23);
			break;
		case 1:
			Decrease(&Now.Minute, 59);
			break;
		case 2:
			Decrease(&Now.Second, 59);
			break;
		default:
			break;
		}
...
void P_Setting_JoystickControl(int joystickStatus)
{
	...
	case JOYSTICK_UP:
		...
		{
		case 0:
			Increase(&Pump_time.Hour, 23);
			break;
		case 1:
			Increase(&Pump_time.Minute, 59);
			break;
		case 2:
			Increase(&Pump_time.Second, 59);
			break;
		default:
			break;
		}
	...
	case JOYSTICK_DOWN:
		...
		{
		case 0:
			Decrease(&Pump_time.Hour, 23);
			break;
		case 1:
			Decrease(&Pump_time.Minute, 59);
			break;
		case 2:
			Decrease(&Pump_time.Second, 59);
			break;
		default:
			break;
		}
...
void DisplayLightStatus(void)
{
	...
	else if ((light.Parameter <= 1000) && (light.Parameter > 100))
	{
		...
		light_on(1, 28);
		light_on(2, 6);
		GLCD_DrawString(0, 8 * 24, "Light source on.  ");
	}
	else if ((light.Parameter <= 3000) && (light.Parameter > 1000))
	{
		...
		light_on(1, 28);
		light_on(1, 29);
		GLCD_DrawString(0, 8 * 24, "Light source off.  ");
	}
	else
	{
...
void DisplayTempStatus(void)
{
	...
	{
		GLCD_DrawString(0, 7 * 24, "It is freezing....");
		light_off_all();
		light_on(2, 6);
		light_on(1, 28);
		GLCD_DrawString(0, 8 * 24, "Heating up.........");
	}
	else if ((temperature.Parameter <= 3000) && (temperature.Parameter > 1000))
	{
		GLCD_DrawString(0, 7 * 24, "Nice weather....");
		light_off_all();
		light_on(2, 6);
		light_on(2, 5);
		GLCD_DrawString(0, 8 * 24, "No need heat source....");
	}
	else
	{
		GLCD_DrawString(0, 7 * 24, "It is hot....");
		light_off_all();
		light_on(2, 5);
		light_on(2, 6);
		light_on(2, 4);
		light_on(1, 28);
		GLCD_DrawString(0, 8 * 24, "Cooling down temp....");
	}
}
